...
def lnorm(arr_in, inker_shape=(3, 3), threshold=1, mode='valid'):
    ...
    arr_out = arr_slice - arr_sum / inker_size
    arr_out = ne.evaluate('arr_slice - arr_sum / inker_size')

    # -- divide by the euclidean norm
    l2norms = (arr_ssq - (arr_sum ** 2.) / inker_size).clip(0, np.inf)
    l2norms = np.sqrt(l2norms)
    l2norms = l2norms.clip(threshold, np.inf)
